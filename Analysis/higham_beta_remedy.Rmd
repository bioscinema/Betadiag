...
for (method in names(distance_matrices)) {
  ...
  dist_gram = Remedy_Gram(dist_gram, "Higham")
...
print(results_table)
write.csv(results_table, "Remedial_results_table_covid.csv", row.names = TRUE)
```
...
# Display the results table
print(results_table)
write.csv(results_table, "Remedial_results_table_cardiovascular.csv", row.names = TRUE)
```
...
# Display the results table
print(results_table)
write.csv(results_table, "Remedial_results_table_IBD_16s.csv", row.names = TRUE)
```
...
# Display the results table
print(results_table)
write.csv(results_table, "Remedial_results_table_IBD_wgs.csv", row.names = TRUE)
```
...
# Display the results table
print(results_table)
write.csv(results_table, "Remedial_results_table_Pig_gut.csv", row.names = TRUE)
```
...
# Display the results table
print(results_table)
write.csv(results_table, "Remedial_results_table_HIV_gut.csv", row.names = TRUE)
```
...
# Display the results table
print(results_table)
write.csv(results_table, "Remedial_results_table_Concordance_oral_16S.csv", row.names = TRUE)
```
...
files <- list.files(pattern = "^Remedial_results_table_.*\\.csv$", full.names = TRUE)
...
df_list <- lapply(files, function(f) {
  ...
  df_name <- sub("Remedial_results_table_(.*)\\.csv", "\\1", basename(f))
...
head(combined_df)
write.csv(combined_df, "higham_filtered_result.csv", row.names = TRUE)
```


```{r}
library(dplyr)
data = read.csv("higham_filtered_result.csv")
...
write.csv(filtered_data, "filtered_data_Remedial.csv", row.names = FALSE)
```
...
# Read the CSV files
combined_result <- read.csv("higham_filtered_result.csv", stringsAsFactors = FALSE)
filtered_data <- read.csv("filtered_data.csv", stringsAsFactors = FALSE)

# Perform the merge based on 'Dissimilarity' and 'source'
merged_data <- inner_join(filtered_data, combined_result,  by = c("Dissimilarity", "source"))

# Save the merged data to a new CSV file
write.csv(merged_data, "merged_data_Higham.csv", row.names = FALSE)
...
```{r}
ggplot(merged_data, aes(x = PValueAdonis.x, y = PValueAdonis.y)) +
  geom_point(alpha = 0.7, color = "blue") +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +  # Add y=x line
  labs(x = "PValueAdonis.x", y = "PValueAdonis.y", title = "Scatter Plot") +
  theme_minimal()
