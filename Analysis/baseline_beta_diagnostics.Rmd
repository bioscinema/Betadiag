...
permanova_gower <- function(G, metadata, nperm = 999) {
  ...
  pseudo_R2 = sum(diag(H %*% Gn %*% H))/sum(diag(Gd))
...
for (method in names(distance_matrices)) {
  ...
  if (!is.null(dist_matrix)) {
    ...
    gower_centered = Dissimilarity_to_Gram(as.matrix(dist_matrix))
    adonis_results_all_covid[[method]] <- permanova_gower(gower_centered, metadata)

    MiRKAT_covid[[method]] <- MiRKAT_R(gower_centered,as.data.frame(metadata[,1]),metadata[,-1])
  }
}
...
print(results_table)
write.csv(results_table, "results_table_covid.csv", row.names = TRUE)
```
...
for (method in distance_methods) {
  ...
  gower_centered = Dissimilarity_to_Gram(as.matrix(dist_matrix))
    adonis_results_cardiovascular[[method]] <- permanova_gower(gower_centered, metadata)

    MiRKAT_cardiovascular[[method]] <- MiRKAT_R(gower_centered,as.data.frame(metadata[,1]),metadata[,-1])
}
...
for (method in methods) {
  # Safely get is_metric
  is_metric <- if (!is.null(Triangle_inequality_cardiovascular[[method]]$is.metric)) {
    ifelse(Triangle_inequality_cardiovascular[[method]]$is.metric == 1, "YES", "NO")
  } else {
    NA_character_
  }
  
  # Safely get neg_counter
  neg_counter <- if (!is.null(Triangle_inequality_cardiovascular[[method]]$neg.counter)) {
    as.numeric(Triangle_inequality_cardiovascular[[method]]$neg.counter)
  } else {
    NA_real_
  }
  
  # Safely get zero_counter
  zero_counter <- if (!is.null(Triangle_inequality_cardiovascular[[method]]$zero.counter)) {
    as.numeric(Triangle_inequality_cardiovascular[[method]]$zero.counter)
  } else {
    NA_real_
  }
  
  # Safely get pcoa_neg_frac
  pcoa_neg_frac <- if (!is.null(pcoa_frac_cardiovascular[[method]])) {
    paste0(pcoa_frac_cardiovascular[[method]], "%")
  } else {
    NA_character_
  }
  
  # Safely extract Adonis2 values (or NA if missing)
...
print(results_table)
write.csv(results_table, "results_table_cardiovascular.csv", row.names = TRUE)
...
for (method in distance_methods) {
  ...
  # Run PERMANOVA
   gower_centered = Dissimilarity_to_Gram(as.matrix(dist_matrix))
    adonis_results_IBD_16s[[method]] <- permanova_gower(gower_centered, metadata)

    MiRKAT_IBD_16s[[method]] <- MiRKAT_R(gower_centered,as.data.frame(metadata[,1]),metadata[,-1])
}
...
# Display the results table
print(results_table)
write.csv(results_table, "results_table_IBD_16s.csv", row.names = TRUE)
```
...
for (method in distance_methods) {
  ...
  # Run PERMANOVA
   gower_centered = Dissimilarity_to_Gram(as.matrix(dist_matrix))
    adonis_results_IBD_wgs[[method]] <- permanova_gower(gower_centered, metadata)

    MiRKAT_IBD_wgs[[method]] <- MiRKAT_R(gower_centered,as.data.frame(metadata[,1]),metadata[,-1])
}
...
# Display the results table
print(results_table)
write.csv(results_table, "results_table_IBD_wgs.csv", row.names = TRUE)
```
...
for (method in distance_methods) {
  ...
  # Run PERMANOVA
   gower_centered = Dissimilarity_to_Gram(as.matrix(dist_matrix))
    adonis_results_Pig_gut[[method]] <- permanova_gower(gower_centered, metadata)

    MiRKAT_Pig_gut[[method]] <- MiRKAT_R(gower_centered,as.data.frame(metadata[,1]),as.data.frame(metadata[,-1]))
}
...
# Display the results table
print(results_table)
write.csv(results_table, "results_table_Pig_gut.csv", row.names = TRUE)
```
...
for (method in distance_methods) {
  ...
  # Run PERMANOVA
   gower_centered = Dissimilarity_to_Gram(as.matrix(dist_matrix))
    adonis_results_HIV_gut[[method]] <- permanova_gower(gower_centered, metadata)

    MiRKAT_HIV_gut[[method]] <- MiRKAT_R(gower_centered,as.data.frame(metadata[,1]),as.data.frame(metadata[,-1]))
}
...
# Display the results table
print(results_table)
write.csv(results_table, "results_table_HIV_gut.csv", row.names = TRUE)
```
...
for (method in distance_methods) {
  ...
  # Run PERMANOVA
  gower_centered = Dissimilarity_to_Gram(as.matrix(dist_matrix))
    adonis_results_Concordance_oral_16S[[method]] <- permanova_gower(gower_centered, metadata)

    MiRKAT_Concordance_oral_16S[[method]] <- MiRKAT_R(gower_centered,as.data.frame(metadata[,1]),as.data.frame(metadata[,-1]))
}
...
# Display the results table
print(results_table)
write.csv(results_table, "results_table_Concordance_oral_16S.csv", row.names = TRUE)
```
...
files <- list.files(pattern = "^results_table_.*\\.csv$", full.names = TRUE)
...
head(combined_df)
write.csv(combined_df, "Combined_result_table.csv", row.names = TRUE)
```


```{r}
library(dplyr)
data = read.csv("Combined_result_table.csv")
...
write.csv(filtered_data, "baseline_filtered_data.csv", row.names = FALSE)
```
